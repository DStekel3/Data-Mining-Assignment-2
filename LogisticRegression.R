...
reviews3.glmnet <- cv.glmnet(train.dat,
                             trainset.labels,
                             family = "binomial",
                             type.measure = "class")
...
reviews.logreg3.pred <- predict(reviews3.glmnet,
                                newx = test3.dat,
                                s = "lambda.1se",
                                type = "class")
...
UsingTermFrequencies <-
  function(trainset,
    ...
    train2.dtm <-
      matrix(as.numeric(train2.dtm > 0), nrow = 640, ncol = 321)
